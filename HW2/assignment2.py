...
def prepare_data(df_train: pd.DataFrame, df_test: pd.DataFrame) -> tuple:
    ...
    '''
    traind = df_train.dropna().to_numpy()
    testd = df_test.dropna().to_numpy()
    train_data = np.array(np.split(traind[:,0], traind[:,0].size))
    test_data = np.array(np.split(testd[:,0], testd[:,0].size))

    return (train_data, traind[:, 1], test_data, testd[:, 1])
    pass
...
def train_test_folds(skf, num_of_folds: int, features: pd.DataFrame, label: pd.Series) -> Tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
    ...
    for train_index, test_index in skf.split(features, label):
        ...
        y_train, y_test = label.iloc[train_index].values.ravel(), label.iloc[test_index].values.ravel()
        ...
        f1_dict['log_reg'].append(metrics.f1_score(y_test, logReg.predict(X_test).round()))
        f1_dict['linear_reg'].append(metrics.f1_score(y_test, linearReg.predict(X_test).round()))
